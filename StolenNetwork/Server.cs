...
namespace StolenNetwork
{
	public class Server<TConnection> : Network<TConnection> where TConnection : IConnection, new()
	{
		...
		public IReadOnlyCollection<TConnection> Connections => _connections.AsReadOnly(); // TODO: Cache?
		...
		private bool ProcessRakNetPacket(byte packetId, TConnection connection, PacketReader reader)
		{
			...
			else if (packetType != RakPacketType.NEW_INCOMING_CONNECTION)
			{
				if (packetType != RakPacketType.DISCONNECTION_NOTIFICATION)
				{
					...
					Logs.Info($"Connection {connection}: Timed out");
				...
				if (connection != null)
				{
					...
					Logs.Info($"Connection {connection}: {connection.DisconnectReason}");
				}
			...
			if (connection == null)
			{
				...
				{
					throw new InvalidOperationException("Cannot start Handshake message in Server.ProcessConnectedPacket");
				}
